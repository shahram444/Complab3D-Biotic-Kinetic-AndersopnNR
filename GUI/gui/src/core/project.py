...
class SimulationMode:
    """Simulation mode flags matching C++ solver."""
    biotic_mode: bool = False
...
class EquilibriumSettings:
    """Equilibrium chemistry solver settings."""
    enabled: bool = False
...
class CompLaBProject:
    ...
    def validate(self) -> List[str]:
        """Comprehensive validation of project settings.

        Checks every aspect that the C++ solver (complab_functions.hh)
        expects to be correct in the XML configuration.
        """
        errors = []

        # ── 1. Domain ────────────────────────────────────────────────
        if self.domain.nx <= 0 or self.domain.ny <= 0 or self.domain.nz <= 0:
            errors.append("Domain: dimensions (nx, ny, nz) must all be > 0")
        if self.domain.dx <= 0 or self.domain.dy <= 0 or self.domain.dz <= 0:
            errors.append("Domain: grid spacing (dx, dy, dz) must all be > 0")
        if self.domain.characteristic_length <= 0:
            errors.append("Domain: characteristic_length must be > 0")
        if not self.domain.geometry_file:
            errors.append("Domain: no geometry file specified")
        elif self.path:
            geom_path = os.path.join(self.get_input_dir(), self.domain.geometry_file)
            if not os.path.isfile(geom_path):
                errors.append(
                    f"Domain: geometry file not found: {self.domain.geometry_file}"
                )

        # Material numbers must be distinct
        mats = {
            self.domain.pore_material,
            self.domain.solid_material,
            self.domain.bounce_back_material,
        }
        if len(mats) < 3:
            errors.append(
                "Domain: pore, solid, and bounce-back material numbers must be distinct"
            )

        # ── 2. Fluid / LBM ──────────────────────────────────────────
        if self.fluid.tau <= 0.5:
            errors.append(
                f"Fluid: tau={self.fluid.tau} is invalid; must be > 0.5 for LBM stability"
            )
        if self.fluid.delta_p < 0:
            errors.append("Fluid: delta_P must be >= 0")
        if self.fluid.peclet < 0:
            errors.append("Fluid: Peclet number must be >= 0")

        # ── 3. Iterations ────────────────────────────────────────────
        if self.iterations.ns_max_iT1 <= 0:
            errors.append("Iterations: ns_max_iT1 must be > 0")
        if self.iterations.ns_max_iT2 <= 0:
            errors.append("Iterations: ns_max_iT2 must be > 0")
        if self.iterations.ns_converge_iT1 <= 0:
            errors.append("Iterations: ns_converge_iT1 must be > 0")
        if self.iterations.ns_converge_iT2 <= 0:
            errors.append("Iterations: ns_converge_iT2 must be > 0")
        if self.iterations.ns_update_interval <= 0:
            errors.append("Iterations: ns_update_interval must be > 0")

        has_transport = len(self.substrates) > 0 or len(self.microbes) > 0
        if has_transport:
            if self.iterations.ade_max_iT <= 0:
                errors.append(
                    "Iterations: ade_max_iT must be > 0 when substrates/microbes present"
                )
            if self.iterations.ade_converge_iT <= 0:
                errors.append("Iterations: ade_converge_iT must be > 0")
            if self.iterations.ade_update_interval <= 0:
                errors.append("Iterations: ade_update_interval must be > 0")

        # ── 4. Simulation mode consistency ───────────────────────────
        mode = self.simulation_mode
        if mode.biotic_mode and mode.enable_abiotic_kinetics:
            errors.append(
                "Mode: biotic_mode and enable_abiotic_kinetics cannot both be true"
            )
        if mode.biotic_mode and len(self.microbes) == 0:
            errors.append("Mode: biotic_mode is enabled but no microbes defined")
        if mode.enable_kinetics and len(self.substrates) == 0:
            errors.append("Mode: enable_kinetics is enabled but no substrates defined")

        # ── 5. Substrates ────────────────────────────────────────────
        sub_names = set()
        for i, sub in enumerate(self.substrates):
            tag = f"Substrate[{i}] '{sub.name}'"
            if not sub.name or not sub.name.strip():
                errors.append(f"Substrate[{i}]: name is empty")
            elif sub.name in sub_names:
                errors.append(f"{tag}: duplicate name")
            sub_names.add(sub.name)

            if sub.diffusion_coefficients.in_pore <= 0:
                errors.append(f"{tag}: diffusion_in_pore must be > 0")
            if sub.diffusion_coefficients.in_biofilm < 0:
                errors.append(f"{tag}: diffusion_in_biofilm must be >= 0")
            if sub.initial_concentration < 0:
                errors.append(f"{tag}: initial_concentration must be >= 0")

        # ── 6. Microbes ──────────────────────────────────────────────
        mic_names = set()
        for i, mic in enumerate(self.microbes):
            tag = f"Microbe[{i}] '{mic.name}'"
            if not mic.name or not mic.name.strip():
                errors.append(f"Microbe[{i}]: name is empty")
            elif mic.name in mic_names:
                errors.append(f"{tag}: duplicate name")
            mic_names.add(mic.name)

            if len(mic.half_saturation_constants) != len(self.substrates):
                errors.append(
                    f"{tag}: has {len(mic.half_saturation_constants)} half-saturation "
                    f"constants but {len(self.substrates)} substrates exist"
                )
            for j, ks in enumerate(mic.half_saturation_constants):
                if ks < 0:
                    errors.append(f"{tag}: half_saturation_constants[{j}] must be >= 0")

            if len(mic.maximum_uptake_flux) < 1:
                errors.append(f"{tag}: maximum_uptake_flux must have at least 1 entry")

            if mic.decay_coefficient < 0:
                errors.append(f"{tag}: decay_coefficient must be >= 0")

            if mic.solver_type == SolverType.CELLULAR_AUTOMATA:
                if self.microbiology.maximum_biomass_density <= 0:
                    errors.append(
                        f"{tag}: CA solver requires maximum_biomass_density > 0"
                    )
                if mic.material_number in (
                    self.domain.pore_material,
                    self.domain.solid_material,
                    self.domain.bounce_back_material,
                ):
                    errors.append(
                        f"{tag}: material_number={mic.material_number} conflicts "
                        f"with domain material numbers"
                    )

        # ── 7. Microbiology global settings ──────────────────────────
        if len(self.microbes) > 0:
            if self.microbiology.maximum_biomass_density <= 0:
                errors.append("Microbiology: maximum_biomass_density must be > 0")
            if not (0 < self.microbiology.thrd_biofilm_fraction < 1):
                errors.append(
                    "Microbiology: thrd_biofilm_fraction must be between 0 and 1"
                )
            if self.microbiology.ca_method not in ("fraction", "half", "none"):
                errors.append(
                    f"Microbiology: ca_method '{self.microbiology.ca_method}' invalid; "
                    f"use 'fraction', 'half', or 'none'"
                )

        # ── 8. Equilibrium ───────────────────────────────────────────
        eq = self.equilibrium
        if eq.enabled:
            if len(eq.component_names) == 0:
                errors.append("Equilibrium: enabled but no component names set")
            n_sub = len(self.substrates)
            n_comp = len(eq.component_names)
            if len(eq.stoichiometry) != n_sub:
                errors.append(
                    f"Equilibrium: stoichiometry has {len(eq.stoichiometry)} rows "
                    f"but {n_sub} substrates exist"
                )
            for row_i, row in enumerate(eq.stoichiometry):
                if len(row) != n_comp:
                    errors.append(
                        f"Equilibrium: stoichiometry row {row_i} has {len(row)} "
                        f"columns but {n_comp} components defined"
                    )
            if len(eq.log_k) != n_sub:
                errors.append(
                    f"Equilibrium: log_k has {len(eq.log_k)} entries "
                    f"but {n_sub} substrates exist"
                )

        # ── 9. I/O ───────────────────────────────────────────────────
        if self.io.save_vtk_interval <= 0:
            errors.append("IO: save_vtk_interval must be > 0")
        if self.io.save_chk_interval <= 0:
            errors.append("IO: save_chk_interval must be > 0")

        # ── 10. Path settings ────────────────────────────────────────
        if not self.input_path:
            errors.append("Paths: input_path is empty")
        if not self.output_path:
            errors.append("Paths: output_path is empty")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CompLaBProject':
        ...
        if "equilibrium" in data:
            project.equilibrium = EquilibriumSettings(**data["equilibrium"])
