...
int main(int argc, char **argv) {
    ...
    char *main_path = (char*)malloc(2048 * sizeof(char));
    getcwd(main_path, 2048 * sizeof(char));
    char *src_path = (char*)malloc(2048 * sizeof(char));
    char *input_path = (char*)malloc(2048 * sizeof(char));
    char *output_path = (char*)malloc(2048 * sizeof(char));
    char *ns_filename = (char*)malloc(2048 * sizeof(char));
    ...
    if (S_ISDIR(statStruct.st_mode)) {} else {
#ifdef _WIN32
        _mkdir(output_path);
#else
    ...
    // Allocate enough space: inputDir + ns_filename + ".chk" + null terminator
    size_t _ns_full_len = str_inputDir.size() + strlen(ns_filename) + 8;
    char *ns_read_filename = (char*)malloc(_ns_full_len * sizeof(char));
    strcpy(ns_read_filename, str_inputDir.c_str());
    strcat(ns_read_filename, ns_filename);
    ...
    T refOmega = 1/refTau;
    std::cout << "  [DBG3] vec_solute_poreD sz=" << vec_solute_poreD.size() << " bFilmD sz=" << vec_solute_bFilmD.size() << "\n" << std::flush;
    T ade_dt = refNu * dx * dx / vec_solute_poreD[0];

    std::cout << "  [DBG3] substrOmega loop\n" << std::flush;
    ...
    std::cout << "  [DBG3] vec_substr_lattices\n" << std::flush;
    ...
    std::cout << "  [DBG3] dC0 ctor\n" << std::flush;
    std::vector< MultiBlockLattice3D<T,RXNDES> > dC0(num_of_substrates, substrLattice);
    std::cout << "  [DBG3] soluteDomainSetup loop\n" << std::flush;
    for (plint iS = 0; iS < num_of_substrates; ++iS) {
        std::cout << "  [DBG3] soluteDomainSetup vec_substr iS=" << iS << "\n" << std::flush;
        ...
        std::cout << "  [DBG3] soluteDomainSetup dC iS=" << iS << "\n" << std::flush;
    ...
    std::cout << "  [DBG3] dC0=dC\n" << std::flush;
    dC0=dC;
    std::cout << "  [DBG3] biomass lattices\n" << std::flush;

    // Create biomass lattices
    // NOTE: use refOmega (not 0.) to avoid inf/NaN in evaluateStatistics() on Windows
    pcout << "  [ADE] Creating " << bfilm_count << " biofilm + " << bfree_count << " planktonic lattices...\n";
    MultiBlockLattice3D<T,RXNDES> initbFilmLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    MultiBlockLattice3D<T,RXNDES> copybFilmLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    MultiBlockLattice3D<T,RXNDES> initbFreeLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    MultiBlockLattice3D<T,RXNDES> copybFreeLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    ...
    std::cout << "  [DBG4] before totalbFilmLattice\n" << std::flush;
    // NOTE: use refOmega (not 0.) to avoid inf/NaN in evaluateStatistics() on Windows
    MultiBlockLattice3D<T,RXNDES> totalbFilmLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    ...
    // NOTE: use refOmega (not 0.) to avoid inf/NaN in evaluateStatistics() on Windows
    std::cout << "  [DBG4] before maskLattice\n" << std::flush;
    MultiBlockLattice3D<T,RXNDES> maskLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    std::cout << "  [DBG4] before ageLattice\n" << std::flush;
    MultiBlockLattice3D<T,RXNDES> ageLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    std::cout << "  [DBG4] before distLattice\n" << std::flush;
    MultiBlockLattice3D<T,RXNDES> distLattice(nx, ny, nz, new AdvectionDiffusionBGKdynamics<T,RXNDES>(refOmega));
    std::cout << "  [DBG4] all lattices created\n" << std::flush;
    std::cout << "  [DBG5] defineMaskLatticeDynamics bio_dynamics.size()=" << bio_dynamics.size() << "\n" << std::flush;
    // In abiotic mode (no microbes), totalbFilmLattice is zero everywhere, which would
    // make defineMaskLatticeDynamics override every maskLattice cell with omega=0
    // dynamics. That triggers NaN/Inf in evaluateStatistics() on Windows (same issue
    // documented above) and causes a downstream access violation. The maskLattice was
    // already constructed with refOmega on line 581, so it's safe to skip the override
    // entirely when no biofilm is present.
    if (num_of_microbes > 0) {
        defineMaskLatticeDynamics(totalbFilmLattice, maskLattice, thrd_bFilmFrac);
    }
    std::cout << "  [DBG5] CopyGeometryScalar2maskLattice3D\n" << std::flush;
    ...
    std::cout << "  [DBG5] CopyGeometryScalar2ageLattice3D\n" << std::flush;
    ...
    std::cout << "  [DBG5] CopyGeometryScalar2distLattice3D\n" << std::flush;
    ...
    std::cout << "  [DBG5] all functional done\n" << std::flush;
