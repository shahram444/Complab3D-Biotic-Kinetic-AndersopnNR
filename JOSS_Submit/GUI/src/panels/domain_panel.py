...
class DomainPanel(BasePanel):
    ...
    def load_from_project(self, project):
        ...
        # Restore the full path the user previously chose so preview works
        if hasattr(d, 'geometry_filepath') and d.geometry_filepath:
            self._geom_filepath = d.geometry_filepath
            self._try_load_preview()
    ...
    def save_to_project(self, project):
        ...
        d.geometry_filepath = self._geom_filepath  # full path, persisted in project
