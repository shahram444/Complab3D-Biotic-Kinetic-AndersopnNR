...
class ChemistryPanel(BasePanel):
    ...
    def _build_ui(self):
        ...
        self._d_biofilm.setToolTip(
            "Effective diffusion coefficient inside biofilm.\n"
            "Typically 0.2–0.5× the pore value.\n"
            "Only used when biotic_mode=true (ignored in abiotic/tracer runs).\n"
            "The XML field is always written but the solver ignores it when\n"
            "no microbes are present.")
        self._d_biofilm.valueChanged.connect(self._on_edited)
        form.addRow("Diffusion in biofilm\n(biotic only):", self._d_biofilm)
