...
class CompLaBMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._project_file = ""
        self._project_dir = ""   # root folder of the current project
        self._runner = None
    ...
    def _new_project(self):
        ...
        if dlg.exec():
            ...
            if chosen_dir and os.path.isdir(chosen_dir):
                ...
                self._scaffold_project_dir(proj_dir)
    ...
    def _scaffold_project_dir(self, proj_dir: str):
        """Create the standard subfolder layout for a new project.

        Layout created:
            <proj_dir>/
            ├── input/
            │   └── geometry.dat   (sample geometry, if available)
            ├── output/            (empty — solver writes VTK/checkpoint files here)
            ├── defineKinetics.hh         (if template provides one)
            └── defineAbioticKinetics.hh  (if template provides one)

        CompLaB.xml is NOT written here; it is written on File > Export XML.
        The kinetics .hh files live at the project root so the user can copy
        them to the solver source tree (same directory as CMakeLists.txt) and
        recompile without hunting through subfolders.
        """
        import shutil
        ...
        # Copy sample geometry.dat from JOSS_Submit/input/ if it exists
        sample_geom = (
            Path(__file__).resolve().parent.parent.parent / "input" / "geometry.dat"
        )
        dest_geom = os.path.join(input_dir, "geometry.dat")
        if sample_geom.exists() and not os.path.exists(dest_geom):
            try:
                shutil.copy2(str(sample_geom), dest_geom)
                self._console.log_info(
                    f"Copied sample geometry.dat → {dest_geom}")
            except OSError as e:
                self._console.log_warning(
                    f"Could not copy sample geometry: {e}")

        # Write kinetics .hh files to project root
        if getattr(self._project, "kinetics_source", ""):
            kin_path = os.path.join(proj_dir, "defineKinetics.hh")
            try:
                with open(kin_path, "w", encoding="utf-8") as f:
                    f.write(self._project.kinetics_source)
                self._console.log_info(f"Written defineKinetics.hh → {kin_path}")
            except OSError as e:
                self._console.log_warning(f"Could not write defineKinetics.hh: {e}")

        if getattr(self._project, "abiotic_kinetics_source", ""):
            akin_path = os.path.join(proj_dir, "defineAbioticKinetics.hh")
            try:
                with open(akin_path, "w", encoding="utf-8") as f:
                    f.write(self._project.abiotic_kinetics_source)
                self._console.log_info(
                    f"Written defineAbioticKinetics.hh → {akin_path}")
            except OSError as e:
                self._console.log_warning(
                    f"Could not write defineAbioticKinetics.hh: {e}")
    ...
    def _run_simulation(self):
        ...
        # Clear the console output so each run starts clean
        self._console.clear()
        self._save_panels_to_project()
    ...
    def _open_geometry_generator(self):
        ...
        The output folder defaults to the current project directory so that
        the generated geometry.dat lands in <project>/input/ automatically.
        """
        from .dialogs.geometry_creator_dialog import GeometryCreatorDialog
        dlg = GeometryCreatorDialog(
            self._project, self._config, self,
