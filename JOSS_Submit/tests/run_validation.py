...
def run_simulation(xml_path: Path, build_dir: Path, np: int, work_dir: Path) -> bool:
    ...
    NOTE: complab always reads 'CompLaB.xml' from its current working directory
    (hardcoded in initialize_complab). The xml_path argument is used only to
    ensure the file has been copied as CompLaB.xml into work_dir before calling
    this function.
    """
    exe = ROOT / "complab"
    if not exe.exists():
    ...
    # The solver reads CompLaB.xml from CWD — no XML path argument needed
    cmd = ["mpirun", "--allow-run-as-root", "-np", str(np), str(exe)]
    result = subprocess.run(
        cmd,
        cwd=str(work_dir),
        capture_output=True,
    ...
    try:
        for i, test in enumerate(TESTS):
            ...
            with tempfile.TemporaryDirectory(prefix=f"complab_val{i+1}_") as tmpdir:
                ...
                # Solver reads CompLaB.xml from CWD (hardcoded filename)
                shutil.copy2(xml_src, work / "CompLaB.xml")

                # Copy input directory (geometry files) into temp work dir
                input_src = TEST_CASES / "input"
                input_dst = work / "input"
                if input_src.exists():
                    shutil.copytree(input_src, input_dst)
                else:
                    # Fall back: look for an input/ folder next to the repo root
                    alt_input = ROOT / "input"
                    if alt_input.exists():
                        shutil.copytree(alt_input, input_dst)
