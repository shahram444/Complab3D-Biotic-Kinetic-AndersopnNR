/* CompLaB3D with Diagnostic Prints - Part 1 of 3
 * Combine all 3 parts into one file
 */
...
int main(int argc, char **argv) {
    ...
    // === END OF PART 1 === (continue in Part 2)
        // === PART 2: NS AND ADE LATTICE SETUP ===
    ...
    // === END OF PART 2 === (continue in Part 3)
        ...
        // === PART 3: POINTER VECTORS AND MAIN LOOP ===

    // Pointer vectors (same as original, condensed)
    ...
    for (; iT < ade_maxiTer; ++iT) {
        ...
        if (ade_VTI_iTer > 0 && iT % ade_VTI_iTer == 0) {
            pcout << "\n╔════════════════════════════════════════════════════════════════════════╗\n";
            pcout << "║ ITERATION " << iT << "  |  Time: " << iT*ade_dt << " s\n";
            pcout << "╠════════════════════════════════════════════════════════════════════════╣\n";
            ...
            for (plint iS = 0; iS < num_of_substrates; ++iS) {
                ...
                pcout << "║   " << vec_subs_names[iS] << " min=" << sMin << " max=" << sMax;
                if (sMin < 0) pcout << " [NEG!]";
                pcout << "\n";
            }
            ...
            if (bfilm_count > 0) {
                ...
                for (plint iM = 0; iM < bfilm_count; ++iM) {
                    ...
                    pcout << "║   " << vec_microbes_names[iM] << " max=" << bMax << "/" << max_bMassRho << " growth=" << growth << "%";
                    if (bMax > max_bMassRho) pcout << " [CA!]";
                    pcout << "\n";
                }
            ...
            // Solvers
            pcout << "║ SOLVERS: ";
            if (kns_count > 0) pcout << "[KNS] ";
            if (useEquilibrium) pcout << "[EQ] ";
            if (ca_count > 0) pcout << "[CA] ";
            pcout << "\n╚════════════════════════════════════════════════════════════════════════╝\n";
    ...
    pcout << "╠══════════════════════════════════════════════════════════════════════════╣\n";
    pcout << "║ Iterations: " << iT << "   Sim time: " << iT*ade_dt << " s\n";
    ...
    if (bfilm_count > 0) {
        ...
        pcout << "║ Final biomass: " << finalBmax << " kg/m3 (growth: " << totalGrowth << "%)\n";
        pcout << "║ CA triggers: " << diag_ca_triggers << "   redistributions: " << diag_ca_redistributions << "\n";
    }
    pcout << "╚══════════════════════════════════════════════════════════════════════════╝\n";
