                ...
                for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
                    for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                        ...
                        if ( mask != solid && mask != bb) {
                            // DEBUG: Confirm we entered a reactive voxel
                            static int voxel_count = 0;
                            if (voxel_count < 5) {
                                std::cout << "[run_kinetics] Voxel: absX=" << absX 
                                          << " iY=" << iY << " iZ=" << iZ 
                                          << " | mask=" << mask 
                                          << " | solid=" << solid << " | bb=" << bb << std::endl;
                                voxel_count++;
                            }
                            ...
                            for (plint iS=0; iS<subsNum; ++iS) {
                                ...
                                // DEBUG: Print concentration vector
                                static int conc_count = 0;
                                if (conc_count < 5) {
                                    std::cout << "[run_kinetics] Conc vector: ";
                            ...
                            for (plint iM=0; iM<bioNum; ++iM) {
                                ...
                                // DEBUG: Print biomass vector
                                static int bmass_count = 0;
                                if (bmass_count < 5) {
                                    std::cout << "[run_kinetics] Biomass vector: ";
                                    for (size_t i = 0; i < bmass.size(); ++i) {
                                        std::cout << "B[" << i << "]=" << bmass[i] << " ";
                                    }
                                    std::cout << std::endl;
                                    bmass_count++;
                                }
                            ...
                            // DEBUG: Print rates returned from kinetics
                            static int rate_count = 0;
                            if (rate_count < 5) {
                                std::cout << "[run_kinetics] After defineRxnKinetics:"
                                          << " subsR[0]=" << subs_rate[0] 
                                          << " bioR[0]=" << bio_rate[0] 
                                          << " dt=" << dt << std::endl;
                                rate_count++;
                            ...
                            for (plint iS=0; iS<subsNum; ++iS) {
                                ...
                                if (dC > thrd || dC < -thrd) {
                                    ...
                                    // DEBUG: Print dC update
                                    static int dC_count = 0;
                                    if (dC_count < 5) {
                                        std::cout << "[run_kinetics] Applying dC=" << dC 
                                                  << " (rate=" << subs_rate[iS] << " * dt=" << dt << ")" << std::endl;
                                        dC_count++;
                                    }
                            ...
                            for (plint iB=0; iB<bioNum; ++iB) {
                                ...
                                if (dB > thrd || dB < -thrd) {
                                    // DEBUG: Print dB update
                                    static int dB_count = 0;
                                    if (dB_count < 5) {
                                        std::cout << "[run_kinetics] Applying dB=" << dB 
                                                  << " (rate=" << bio_rate[iB] << " * dt=" << dt << ")" << std::endl;
                                        dB_count++;
                                    }
                    ...
                    for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                        ...
                        if ( mask != solid && mask != bb) {
                            ...
                            for (size_t iB=0; iB<bioNum; ++iB) {
                                ...
                                if (dB > thrd || dB < -thrd) {
                                    Array<T,7> g;
                                    lattices[subsNum+iB]->get(iX+vec_offset[iB].x,iY+vec_offset[iB].y,iZ+vec_offset[iB].z).getPopulations(g);
                                    g[0]+=(T) (dB)/4; g[1]+=(T) (dB)/8; g[2]+=(T) (dB)/8; g[3]+=(T) (dB)/8; g[4]+=(T) (dB)/8; g[5]+=(T) (dB)/8; g[6]+=(T) (dB)/8;
                                    lattices[subsNum+iB]->get(iX+vec_offset[iB].x,iY+vec_offset[iB].y,iZ+vec_offset[iB].z).setPopulations(g);
                                }
                            }
